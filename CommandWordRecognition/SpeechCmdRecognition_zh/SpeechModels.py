...
def AttRNNSpeechModel(nCategories, samplingrate = 16000, inputLength = 16000, rnn_func = CuDNNLSTM):
    ...
    x = Conv2D(20, (5,1) , activation='relu', padding='same') (x)
    x = BatchNormalization() (x)
    x = Conv2D(10, (5,1) , activation='relu', padding='same') (x)
    x = BatchNormalization() (x)
    x = Conv2D(1, (5,1) , activation='relu', padding='same') (x)
    ...
    x = Bidirectional(rnn_func(128, return_sequences = True)) (x) # [b_s, seq_len, vec_dim]
    x = Bidirectional(rnn_func(128, return_sequences = True)) (x) # [b_s, seq_len, vec_dim]

    xFirst = Lambda(lambda q: q[:,128]) (x) #[b_s, vec_dim]
    ...
    x = Dense(128, activation = 'relu')(attVector)
    x = Dense(80)(x)
